...
func newUpstreamProber(cfg config.Config) *upstreamProber {
	// Probe only what traffic actually reaches: a probe against an unrouted upstream spends a live
	// credential on a network call whose answer changes nothing.
	// **只探真有流量的那一个。** 对着一条无流量上游的探测,是拿一把活凭证去打一个「答案改变不了
	// 任何事」的网络请求。
