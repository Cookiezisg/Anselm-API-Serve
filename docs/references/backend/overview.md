...
| media | `app/media` | `POST /v1/media/uploads` · `PUT /v1/media/uploads/{id}` · `POST /complete` | install-bound resumable staging、opaque lease、HMAC-derived private fetch credential、startup/periodic TTL recovery；字节不进 SQLite |
