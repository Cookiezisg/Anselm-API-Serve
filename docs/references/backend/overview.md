...
| media | `app/media` | `POST /v1/media/uploads` · `GET/PUT /v1/media/uploads/{id}` · `POST /complete` | install-bound resumable staging（GET 是 ambiguous PUT 后权威 cursor）、opaque lease、HMAC-derived private fetch credential、startup/periodic TTL recovery；字节不进 SQLite |
