...
| GW-INV-10 | DeepSeek quote=`UTF-8 byte-fallback prompt 上界（含 tools/tool_choice/message tool continuation 与 64 token/message framing）+ clamped output`；Qwen3.7 Plus 视觉输入不能由本地字节数证明 token，reserve 完整 `1,000,000` input + `65,536` output 的最高分档 hard limit；任一已启用 route 的单请求 quote 必须能装入 operator 月预算，否则配置 fail-fast | 请求注定无法 reserve、byte-split tokenizer/tool 字段造成欠预留，或 Qwen hidden thinking 造成欠预留 |
