...
| GW-INV-41 | 运维白名单（`WHITELIST_TOKEN_SHA256`，env-only、空=dormant，凭据派生哈希按机密同纪律：不入 `Specs`/`Dump`，`Snapshot` 只报计数，解析失败**不回显原值**只报位置+长度）：命中 `SHA-256(bearer)` = **god-mode 全风控绕过**——单点 `app/install.IsUnmetered` 判定（chat/quota/install 共用），chat 跳过分钟限速 + 异常降速且 `Reserve(unmetered=true)` 经 `unmeteredLimits()` 把月次数/日 token/**全局钱包**三闸抬到 `config.Max*`、`DailySublimit` 置 0（禁用 gate 2b、不 +1 日计数、`SublimitApplied` 恒 false），`/v1/install` 带白名单 bearer 复领时跳过 PoW + 全部 Sybil 闸（`IssueParams.Unmetered`）。但**仅放宽闸门**：`reserve→settle→rollback` saga + ledger 代码逐字不变（仍记账、GW-INV-01..06 完好，unmetered 仅少跑 gate 2b），仍每次全新 token 行/全新配额池（GW-INV-12 不变），`banned` 仍先于 god-mode，持久化 config 永不被改（SEC-2/GW-INV-10 不变）；god-mode 花费仍计入共享 `budget`（saga 对称所需），失控可把 `tokens_used` 顶到 `GLOBAL_DAILY_BUDGET_TOKENS` 致**他人 `BUDGET_EXHAUSTED`**（已显式记档）；非 64-hex 条目 fail-fast | 白名单 token 外泄 = 无限免费/烧钱 + 耗尽共享日预算拖垮他人；哈希/原值入 `Dump`/日志 = 凭据外泄；绕过 saga 记账 = 账本不守恒 |
