...
| GW-INV-37 | reasoning 档位由服务端唯一确定。caller `max_tokens` 正数 wire 值为 `min(client,MAX_TOKENS_CAP,selected output limit)`，缺省/非正 wire 值为 `min(MAX_TOKENS_CAP,selected output limit)`；DeepSeek quote 用同一 bounded output，Kimi quote 始终按完整 hard limits | payload、账本与客户端 headroom 漂移，输出成本失控，或客户端绕过产品档 |
