...
Anselm 客户端的附件域已经需要统一表达图像、视频与音频；未来上游会改为具备音频能力的模型。但当前网关只有 DeepSeek 文本 route 与 Qwen3.7 Plus 图像/视频 route，Qwen3.7 的视觉 rate card 和 compatibility payload 都不能作为音频计价、路由依据。

把音频继续当作未知输入会迫使客户端另外分叉协议；把它伪装成视觉 route 又会在没有可靠能力、上游 wire 和 rate card 的情况下错误扣费。确定性分流、无 fallback 与 pUSD 冻结账本决策不变。
...
当前路由表显式增加第三个终点：任一合法 audio part（即使同一历史也有图片/视频）在任何 `Reserve`、provider `Open`、wallet 变更之前返回 `503 AUDIO_UNAVAILABLE`。这表示当前无任何可计价、可兼容的音频上游；绝不 fallback。
