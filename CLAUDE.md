...
- **密钥边界**:secret(`DEEPSEEK_API_KEY`/`KIMI_API_KEY`/`DASHBOARD_USER`/`DASHBOARD_PASSWORD`/`INSTALL_POW_SECRET`)env-only，不入 `config.Specs()`、绝不入 settings 表/Dump/Snapshot/日志；Snapshot 只能暴露安全状态/数量。
